...
- `engine/menu.py`：旧版 tkinter 主菜单（保留）
- `engine/game_frame.py`：旧版 tkinter 阅读界面（保留）
- `engine/sidebar_tabs.py`：左侧 Tab 工具栏组件（模块化 UI）
- `engine/effects.py`：文本演出效果实现与注册
- `engine/text_render.py`：Canvas 描边文本渲染工具（创建/就地更新）
...
- 非渐变阶段复用缓存 `PhotoImage`，震动阶段优先通过坐标位移更新，减少逐帧重建导致的闪烁。
- 文本动效仅清理 `text_layer`，不整画布清空，以避免背景快速闪烁。
- 背景图定位使用左上角锚点（`nw`），默认不位移，仅在 `shake` 生效时临时位移，避免非预期像素跳动。

### 6.5 文本可读性约定

- 正文区域不再叠加黑色底板，改为正文黑色描边以突出文字并保留完整背景画面。
- 文本以覆盖方式渲染在背景层上方，不使用底部对话框容器。
- 文本起点固定在中间阅读区左上角内边距区域，并自上向下排版。
- 中间阅读区不再显示剧本标题、操作提示与人物名。
- 右侧人物栏取消说话人字段，仅显示立绘；操作提示迁移到右侧栏下半区域。
- 段内换行优先通过“行间距”呈现，不依赖 `\n\n` 空白行制造视觉间隔。
...
- 当前段最后一步显示完成后，下一次空格/左键才进入下一段并清空文本层。
- 同段后续步骤采用“追加显示”策略，不重复从首字符重播整步动画。
- 同段后续步骤会对“新增文本”播放动效，已显示文本保持静态不重播。
- 动画阶段与静态阶段均使用黑色描边文字渲染，以保持视觉一致性。
- 文本动画渲染采用“同一组 Canvas 文本对象就地更新”策略，避免逐帧 delete/create 导致的闪烁。
...
- 2026-03-02：pygame 细节优化：窗口在“主菜单→阅读页”尺寸切换时保持中心点不变；打字机末尾光标统一为 `|`；并统一 `typewriter` 速度为固定值 `55`（同步 scripts/skills/README/开发文档）。
- 2026-03-02：继续收敛 pygame 阅读 UI：中间阅读区移除标题/操作提示；全局字体统一为微软雅黑；右侧栏补齐外边距与对称布局；右栏取消说话人字段并改为“立绘 + 操作提示”，左栏仅保留进度。
- 2026-03-02：调整 pygame 交互细节：主菜单窗口固定 `1280x720`，进入阅读后再扩展至左右栏布局；中间阅读区取消人物名重复显示；正文排版改为行间距策略，弱化 `\n\n` 空白行依赖。
- 2026-03-02：调整 pygame 阅读布局：左右边栏改为位于中间阅读区外侧（不挤占阅读区宽度），并将正文起点改为中间阅读区左上角，替代底部起始布局。
- 2026-03-02：补齐 pygame 阅读器缺失能力：新增左侧操控栏与右侧人物立绘栏；正文改为背景层上方直接渲染（移除底部对话框样式），保留 `display_break_lines` 分步推进与 `typewriter/shake` 动效。
- 2026-03-02：运行时迁移到 `pygame`：`main.py` 改为启动 `engine/pygame_app.py`，新增 pygame 版菜单与阅读主循环（线性阅读、`display_break_lines` 分步推进、`typewriter/shake` 动效、背景 fade/shake、回退/退出），为后续更游戏化扩展预留基础。
- 2026-03-02：进一步优化抗闪烁：背景层在非渐变阶段改为复用缓存 `PhotoImage` 并仅做坐标位移，文本动效收束阶段改为就地更新静态文本，减少完成帧的清空重绘闪烁。
- 2026-03-02：整理文本渲染结构：新增 `engine/text_render.py` 统一描边文本创建与就地更新逻辑，`game_frame.py` 与 `effects.py` 改为复用公共函数；同步更新 README 与技能说明的一致性表述。
- 2026-03-02：更换文本动画渲染底层逻辑：由逐帧删除重建改为对象就地更新（含分步追加动画），进一步降低动画期闪烁。
- 2026-03-02：移除文字波浪动效：删除 `wave` 动画实现与注册，剧本效果字段统一迁移为 `typewriter`，并同步更新 skill/README/开发文档。
- 2026-03-02：回退“动画阶段不显示描边”策略：恢复动画期间描边渲染（含分步追加动画），与静态文本视觉保持一致。
- 2026-03-02：按需求取消文字渐显动画：移除文字渐显特效逻辑与默认回退，阅读器统一使用非渐显效果；同步更新 scripts、README、skills 与开发文档。
- 2026-03-02：按阅读观感需求移除正文黑底，改为文字黑色描边；并优化空格跳过动画路径，避免“先清空再显示”带来的闪烁。
- 2026-03-02：优化阅读观感：正文底板改为半透明深色遮罩，并将后续步新增文本动画改为单对象增量渲染，减少推进时闪烁。
- 2026-03-02：修复后续步新增文本动画错位：新增文本改为按整段排版坐标逐字渲染，避免换行参考系偏移导致文字歪斜。
- 2026-03-02：增强段内分步体验：同段后续步骤改为“仅新增文本播放动画”，已显示文本保留且不重播。
- 2026-03-02：修复段内推进重播问题：同段进入下一步时改为增量追加显示，不再从头重放已显示文本。
- 2026-03-02：修正段内推进规则为“配置驱动分步追加”：通过段落断点配置控制同段文本追加，取消自动按句切分；空格/左键逐步追加文本，最后一步后再切段。
- 2026-03-02：分步配置升级为“断点模式”：新增 `display_break_lines`（按原文行号断点）以避免重复文本。
- 2026-03-02：阅读器新增“段内逐句推进”交互：同段内空格/左键按句推进，句块播放完后再切段；动画中可继续空格/左键跳过当前句动画。
- 2026-03-02：剧本与生产规范统一为视觉小说线性形态：重写 `scripts/今天也在摸鱼.json`、`scripts/午夜密室.json` 为自然单线叙事；同步 skill 与文档默认 `shared.planning.script_form=visual_novel`。
...
- 2026-02-27：选项区升级为“按需显示”的半透明浮层：仅在存在 `choices` 时显示并覆盖在画布底部，无交互跳转字段时完全隐藏。
