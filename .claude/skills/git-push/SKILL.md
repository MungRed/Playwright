...
3. 根据用户回答执行：
   - **单次提交模式**
     - 按用户选择暂存文件
     - 若用户未提供提交信息，自动生成 Conventional Commits 风格信息（默认优先中文）
...
- 自动生成时优先使用中文提交信息；若用户明确要求英文，则使用英文。
- 使用中文或英文均可，与项目保持一致。
...
- `fix:` 修复 Bug
- `docs:` 文档变更
- `chore:` 构建流程/配置/工具变更（含 `style:` / `refactor:` / `test:`）

## 拆分策略（推荐）

- 优先按**改动目的**拆分：`feat` / `fix` / `docs` / `chore`
- 其次按**模块或目录**拆分：如 `engine`、`.claude/skills`、`docs`
...
```bash
# 1) 引擎功能改动
git add engine/pygame_app.py main.py
git commit -m "feat(engine): 优化 pygame 阅读器交互"

# 2) 流程规范与技能改动
git add .github/copilot-instructions.md .claude/skills
git commit -m "docs(workflow): 统一三模块 agent 编排规范"

# 3) 文档改动
git add README.md docs/DEVELOPMENT.md docs/PITFALLS.md
git commit -m "docs(project): 同步说明与开发约束"
