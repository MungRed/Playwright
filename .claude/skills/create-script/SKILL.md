...
- 当上下文过长时，必须改用 `context_files`（本地路径或 URL）上传到混元文件接口，并通过消息 `FileIDs` 参与对话；**必须设置 `enable_deep_read=false`**（`enable_deep_read=true` 对应的深度阅读功能默认未开通，传 `true` 会导致 `InvalidParameter` 报错）。
- **混元文件接口仅支持 `.txt` 格式**（不支持 `.md`）；上传前必须将草稿另存为 `.txt` 文件再传入 `context_files`。
...
- 推荐文件：
	- 阶段1规划草稿：`planning_draft.md`（可读性存档）+ `planning_draft.txt`（混元上传用）
	- 阶段2正文草稿：`novel_draft.md`（可读性存档）+ `novel_draft.txt`（混元上传用）
- **当需要通过 `context_files` 上传草稿时，必须使用 `.txt` 副本**；混元文件接口不支持 `.md` 格式，上传 `.md` 会导致 `InvalidParameterValue` 错误。
...
- 若 `planning_draft.txt` 全文 + 已生成正文超出单次消息预算，必须将两者作为 `context_files` 上传后续写，禁止仅传"任务：续写xx段"而不附前文。
