...
- 允许在调用前后做结构化处理（提示词拼装、JSON 校验、字段修复），但创作文本本体必须来自生文 API 返回。
...
3. 生成脚本结构（严格 JSON）：
	- 先组织生文提示词，调用 `mcp_playwright-im_generate_text` 生成：
		- `shared.planning.requirements_summary/worldview/characters/outline`
		- `segments[].text`（以及可选 `speaker`）
	- 将 API 返回内容映射到剧本 JSON 结构
