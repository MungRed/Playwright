...
4.0 图生图前置检查（必须执行）：
   - 图生图前必须先确保 `reference_images` 全部可通过 `https` 访问。
   - 若输入为本地路径，必须先上传 COS（或复用已存在对象）并拿到 COS URL，再调用图生图接口。
   - 调用 `SubmitTextToImageJob` 时，`reference_images` 仅允许传 URL，不得直接传本地路径。

4.1 立绘生成约束：
   - 角色立绘强制使用角色设定图作为参考（图生图），禁止退回纯文生图。
   - 角色立绘强制使用混元 3.0 图生图接口：`api_action=SubmitTextToImageJob`。
   - 至少生成“中性情绪”立绘；剧情存在明显情绪变化时增量生成 `calm/happy/serious/...`。
   - 命名：`char_<name>_<mood>.png`，不得将 `char_ref_*` 直接当立绘回写。
   - 立绘分辨率必须为竖向（推荐 `720x1280`，或等比例竖图），不得输出横向立绘。
...
6. 输出资产清单：
   ...
   - **路径必须使用生图工具实际返回的文件名**（或 `glob`/`os.listdir` 从磁盘读取），不得按角色名推断（防止汉字拼写差异导致路径悬空）
...
- 图生图严格执行“先 COS 后图生图”：先上传/复用 COS 获取 URL，再请求生成。
- 图生图调用失败（限流/参考图不可访问/任务超时）时，必须中断自动流程并询问用户“重试 / 降级 / 终止”，不得静默退回文生图。
