...
4. 一致性校验：
   - asset_manifest 中的 `segment_id` 必须与 `segments[i].id` 字段值严格匹配（如 `"s1"` 对应段落 `"id": "s1"`），**不得使用数字字符串索引**（`"0"`, `"1"` 等）
   - 路径写回前必须通过 `os.path.exists` 或 `glob` 核验目标文件实际存在（防止角色名拼写与实际文件名不一致导致路径悬空）
   - 路径建议使用相对路径 `assets/...`
   - 不创建无映射的空字段
