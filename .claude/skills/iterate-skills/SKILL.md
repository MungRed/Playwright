...
自动检查 `.claude/skills/**/SKILL.md`，识别可优化点并执行精确聚焦优化，重点关注：
...
2. **项目变更影响分析（必须）**
   ...
   - 触发映射（最低要求）：
      ...
      - 剧本演出规则变化（`effect`/`speed`/`display_break_lines`/背景表现）→ 必须检查并迭代 `configure-script-presentation`（并校验 `typewriter` 固定速度约束）
